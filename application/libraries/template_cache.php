...
class template_cache {
    ...
    public function template_compile($module, $template, $style = 'default') {
        ...
        $filepath = $this->cache_path.DIRECTORY_SEPARATOR_MY.'caches_template'.DIRECTORY_SEPARATOR_MY.$style.DIRECTORY_SEPARATOR_MY.$module.DIRECTORY_SEPARATOR_MY;


        if(!is_dir($filepath)) {
            mkdir($filepath, 0777, true);
        }
        $compiledtplfile = $filepath.$template.TPL_EXT;
    ...
    public static function pc_tag($op, $data, $html) {
        ...
        if (in_array($op,$tools)) {
            ...
            if ( file_exists(APPPATH.'libraries'.DIRECTORY_SEPARATOR_MY.$op.'_tag.php')) {
